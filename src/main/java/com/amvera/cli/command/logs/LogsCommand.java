...
public class LogsCommand {
    ...
    public void logs(
            ...
            @Option(longNames = "type", shortNames = 't', arity = OptionArity.EXACTLY_ONE, description = "Type of logs (build or run)", required = true) String type,
    ...
    @Command(command = "run-logs", description = "Show running project logs")
    @CommandAvailability(provider = "userLoggedOutProvider")
    public void runLogs(
            @Option(longNames = "project", shortNames = 'p', arity = OptionArity.EXACTLY_ONE, description = "Project id, name or slug", required = true) String project,
            @Option(longNames = "limit", shortNames = 'l', arity = OptionArity.EXACTLY_ONE, defaultValue = "50", description = "Log lines limit (max 1000)") Integer limit
    ) {
        logs(project, "run", limit);
    }

    @Command(command = "build-logs", description = "Show running project logs")
    @CommandAvailability(provider = "userLoggedOutProvider")
    public void buildLogs(
            @Option(longNames = "project", shortNames = 'p', arity = OptionArity.EXACTLY_ONE, description = "Project id, name or slug", required = true) String project,
            @Option(longNames = "limit", shortNames = 'l', arity = OptionArity.EXACTLY_ONE, defaultValue = "50", description = "Log lines limit (max 1000)") Integer limit
    ) {
        logs(project, "build", limit);
    }
